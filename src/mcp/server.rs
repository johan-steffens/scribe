...
//! | Reporting | `report_summary`, `report_project`, `report_task` |
//!
//! # Resource URIs
//!
//! | URI | Contents |
...
use crate::ops::{
    InboxOps, NotesOps, ProjectOps, ReminderOps, ReportingOps, TaskOps, TodoOps, TrackerOps,
};
...
/// Parameters for `report_summary`.
#[derive(Debug, Serialize, Deserialize, JsonSchema)]
pub struct ReportSummaryParams {
    /// Restrict to items due or created today.
    pub today: Option<bool>,
    /// Restrict to items due or created this week.
    pub week: Option<bool>,
}

/// Parameters for `report_project`.
#[derive(Debug, Serialize, Deserialize, JsonSchema)]
pub struct ReportProjectParams {
...
/// Parameters for `report_task`.
#[derive(Debug, Serialize, Deserialize, JsonSchema)]
pub struct ReportTaskParams {
...
impl ScribeMcpServer {
    ...
    pub fn new(conn: &Arc<Mutex<Connection>>) -> Self {
        ...
        Self {
            ...
            reporting: ReportingOps::new(Arc::clone(conn)),
            notes: notes_ops,
...
/// A task with an optional `children` array for hierarchical display.
#[derive(Debug, Serialize, Deserialize)]
struct HierarchicalTask {
    #[serde(flatten)]
    task: crate::domain::Task,
    children: Vec<HierarchicalTask>,
}

/// Builds a hierarchical task tree from a flat list of tasks.
///
/// Parent tasks (where `parent_id` is `None`) become roots.
/// Children are recursively nested under their parents.
fn build_task_hierarchy(tasks: Vec<crate::domain::Task>) -> Vec<HierarchicalTask> {
...
impl ScribeMcpServer {
    ...
    ///
    /// Returns a hierarchical JSON structure where parent tasks contain
    /// a `children` array of their nested sub-tasks.
    #[tool(description = "List active Scribe tasks in a hierarchical structure, \
        optionally filtered by project_slug and/or status.")]
    ...
    /// Generates a summary report across all Scribe domains.
    #[tool(description = "Generate a Scribe summary report across all domains. \
    ...
    #[tool(
        description = "Generate a detailed report for a specific Scribe project. \
        Returns the project details, pending tasks, open todos, time entries, \
    ...
    async fn report_project(&self, Parameters(p): Parameters<ReportProjectParams>) -> String {
        ...
        match self.reporting.project_report(&p.slug, since, until) {
            Ok(report) => to_json(&serde_json::json!({
                ...
                "pending_tasks": report.pending_tasks,
                "open_todos": report.open_todos,
                "time_entries": report.time_entries.into_iter().map(|(e, d)| {
                    serde_json::json!({
                        "slug": e.slug,
                ...
                "total_time_seconds": report.total_time.num_seconds(),
    ...
    #[tool(description = "Generate a detailed report for a specific Scribe task.")]
    async fn report_task(&self, Parameters(p): Parameters<ReportTaskParams>) -> String {
        match self.reporting.task_report(&p.slug) {
            Ok(report) => to_json(&serde_json::json!({
                "task": report.task,
                "time_entries": report.time_entries.into_iter().map(|(e, d)| {
                    serde_json::json!({
    ...
    /// Reads a note by its slug.
    #[tool(description = "Read a Scribe note by its slug.")]
    async fn read_note(&self, Parameters(p): Parameters<ReadNoteParams>) -> String {
        match self.notes.get(&p.slug) {
            Ok(Some(note)) => to_json(&note),
            Ok(None) => err_json(format!("note '{}' not found", p.slug)),
            Err(e) => err_json(e),
        }
    }

    /// Creates a new note with the given title and content.
    #[tool(description = "Create a new Scribe note with a title and markdown content.")]
    async fn write_note(&self, Parameters(p): Parameters<WriteNoteParams>) -> String {
        match self.notes.write_note(&p.title, &p.content) {
    ...
    #[tool(description = "Search Scribe notes by full-text query using FTS5. \
        Returns up to 50 matching notes ordered by relevance.")]
    async fn search_notes(&self, Parameters(p): Parameters<SearchNotesParams>) -> String {
        match self.notes.search_notes(&p.query) {
